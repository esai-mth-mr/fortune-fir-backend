...
export const addYearStory = async (req: Request<IReq>, res: Response) => {
    ...
    if (error) {
        return res
            .status(400)
            .json({ error: true, message: error.details.map((err) => err.message) });
    }
    ...
    try {
        ...
        if (!storyData) {
            return res
                .status(404)
                .json({ error: true, message: "Stories not found" });
        }
        ...
        res
            .status(200)
            .json({ error: false, message: "Successfully generated total story" });
    } catch (error: any) {
