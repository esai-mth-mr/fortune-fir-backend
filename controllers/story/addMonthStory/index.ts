...
export const addMonthStory = async (req: Request<IReq>, res: Response) => {
    ...
    try {
        ...
        const assetDocuments = await Asset.find({ index: { $in: assets } }).session(
            session
        );
        ...
        if (!story) {
            ...
            await new Log({
                ...
                reason: existingMonthStory
                    ? "Month Story updated successfully"
                    : "Month Story created successfully",
            }).save({ session });
        }
